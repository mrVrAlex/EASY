...
class DeveloperToolsController  implements Dispatchable
{
    ...
    public function indexAction()
    {
        ...
        $db = $locator->get('db-config');
        /**
         * @var $profiler \Zend\Db\Profiler
         * @var $query \Zend\Db\Profiler\Query
         */
        $profiler = $db->getProfiler();
        $querys = array();
        $sql = $profiler->getQueryProfiles();
        if ($sql) {
        foreach ($sql as $query){
            $querys[] = array('sql'=>$query->getQuery(),'time'=>round($query->getElapsedSecs(),4),'params'=>$query->getQueryParams());
        }
        }
        $totalTimeSql = $profiler->getTotalElapsedSecs();
        $totalCountSql = $profiler->getTotalNumQueries();
        return array('execTime' => round($execTime, 5) * 1000,'querys'=>$querys,'totalCountSql'=>$totalCountSql,'totalTimeSql'=>round($totalTimeSql,4));
    }
    ...
    public function execute(MvcEvent $e)
    {
        ...
        //$db = $locator->get('Zend\Db\Adapter\PdoMysql');
        //$profiler = $db->getProfiler();
