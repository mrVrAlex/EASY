...
class Route implements RouteInterface
{
     ...
     *
     * @param \Zend\Config $config Configuration object
     */
    public static function factory($config = array())
    {
        if(is_array($config) && !$config instanceof Config)
            $config = new Config($config, true);
        $reqs = ($config->reqs instanceof Config) ? $config->reqs->toArray() : array();
    ...
    public function match(Request $request)
    {
        ...
        $request->setMetadata($return);
        $this->_values = $values;
        $matchedLength = count($values);
        return new RouteMatch($return, $matchedLength);
     ...
     */
    public function assemble(array $data = array(), array $options = array())
    {
        $reset = true;
        $encode = false;
        $partial = false;
